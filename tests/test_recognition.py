...
class RecognitionTests(unittest.TestCase):
    def test_training_writes_loadable_model_and_label_mapping(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            ), patch("recognition.LABELS_PATH", labels), patch(
                "recognition._detector", return_value=WholeImageDetector()
            ):
    ...
    def test_largest_face_selection_prefers_the_biggest_region(self) -> None:
        ...
        self.assertEqual(
            recognition._largest_face_rectangles(rectangles), (0, 0, 120, 120)
        )
